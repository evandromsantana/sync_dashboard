...
export const EditorData = () => (
  <div>
    <h3>
      Este projeto desenvolvido em React foi concebido com o propósito de
      facilitar uma adoção gradual da tecnologia. Ao longo do tempo, sua
      utilização pode ser ajustada conforme a necessidade e as interações
      requeridas. A inserção de elementos interativos em uma página web,
      contribui para simplificar a compreensão do conteúdo apresentado. Por meio
      deste sistema baseado em React, os recursos disponíveis nesta seção têm o
      intuito de auxiliar no início da utilização da plataforma. Em síntese, o
      sistema proporciona uma experiência interativa que permite ao usuário
      editar texto, além de possibilitar o download e a edição de arquivos
      diretamente a partir do sistema de arquivos local em seu navegador.
    </h3>
  </div>
);
